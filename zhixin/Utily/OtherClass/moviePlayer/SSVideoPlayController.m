...
- (void)viewDidLoad {
    [super viewDidLoad];
    _mpVC = [[MPMusicPlayerController alloc]init];
    [self setup];
...
//    self.volume.value = [[MPMusicPlayerController applicationMusicPlayer]volume];
    self.volume.value = _mpVC.volume;
...
- (void)addGesture {
    UISwipeGestureRecognizer *progressRight = [[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(progress:)];
    [progressRight setDirection:UISwipeGestureRecognizerDirectionRight];
    UISwipeGestureRecognizer *progressLeft = [[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(progress:)];
    [progressLeft setDirection:UISwipeGestureRecognizerDirectionLeft];
    [self.view addGestureRecognizer:progressRight];
    [self.view addGestureRecognizer:progressLeft];
    UISwipeGestureRecognizer *volumeUp = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(volume:)];
    [volumeUp setDirection:UISwipeGestureRecognizerDirectionUp];
    [self.view addGestureRecognizer:volumeUp];
    UISwipeGestureRecognizer *volumeDown = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(volume:)];
    [volumeDown setDirection:UISwipeGestureRecognizerDirectionDown];
    [self.view addGestureRecognizer:volumeDown];
}

- (void)progress:(UISwipeGestureRecognizer *)swip {
    if (swip.direction == UISwipeGestureRecognizerDirectionRight) {
        self.slider.value +=0.05;
    }else {
        self.slider.value-=0.05;
    }
    [self.player moveTo:self.slider.value];
    [self.player play];
    
}

- (void)volume:(UISwipeGestureRecognizer *)swip {
    if (swip.direction == UISwipeGestureRecognizerDirectionUp) {
        self.volume.value +=0.1f;
        
    }else {
        self.volume.value -=0.1f;
    }
    _mpVC.volume = self.volume.value;
}
